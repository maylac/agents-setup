...
2. **myLife リポジトリ** — `~/workspace/myLife`(個人ライフログの LLM Wiki + 自動同期パイプライン)
...
python3 -c "import json, os; d=json.load(open(os.path.expanduser('~/.claude/settings.local.json'))); p=d.get('permissions',{}); print({k: len(v) for k,v in p.items()})"
```
