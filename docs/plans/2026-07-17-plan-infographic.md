...
- スクリプト shebang は `#!$HOME/.local/pipx/venvs/notebooklm-mcp-cli/bin/python` 固定（システム python には notebooklm_tools が無い）
...
```python
#!$HOME/.local/pipx/venvs/notebooklm-mcp-cli/bin/python
