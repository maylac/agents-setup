...
Otherwise, run: `~/.agents/skills/agmsg/scripts/whoami.sh "$HOME" claude-code`
...
→ Show the user the available teams from the output, then:
  ...
  3. **You MUST use join.sh** — run: `~/.agents/skills/agmsg/scripts/join.sh <team> <agent_name> claude-code "$HOME"`
  ...
  5. **REQUIRED — Do NOT skip this step.** Ask the user to pick a delivery mode using exactly this prompt:
     ...
     - Map the chosen number to a mode and run:
       `~/.agents/skills/agmsg/scripts/delivery.sh set <mode> claude-code "$HOME"`
...
→ No exact registration exists for this project, but there are same-type agent names registered elsewhere.
  ...
  4. Run: `~/.agents/skills/agmsg/scripts/join.sh <team> <agent_name> claude-code "$HOME"`
...
2. Run `~/.agents/skills/agmsg/scripts/identities.sh "$HOME" claude-code` to see whether the role is already registered for this (project, type).
3. If the name does not appear in the output, join under the existing team. Read TEAMS from the in-session whoami state (it may be a single team or comma-separated). For a single team, run `~/.agents/skills/agmsg/scripts/join.sh <team> <name> claude-code "$HOME"`. For multiple teams, ask the user which team to join the new role into, then run join.sh for that team.
4. **Pre-flight claim** the actas exclusivity lock so this role isn't already owned by another live session: `~/.agents/skills/agmsg/scripts/actas-claim.sh "$HOME" claude-code <name> "$CLAUDE_CODE_SESSION_ID"`. Read the `status=` line of the output:
...
5. **Switch receive too — exclusive role mode.**
   ...
   d. **Only if the project's delivery mode is `monitor` or `both`** (check via `~/.agents/skills/agmsg/scripts/delivery.sh status claude-code "$HOME"`), invoke a fresh Monitor, regardless of whether step b or c applied:
      - command: `~/.agents/skills/agmsg/scripts/watch.sh $CLAUDE_CODE_SESSION_ID "$HOME" claude-code <name>`
...
2. Run `~/.agents/skills/agmsg/scripts/reset.sh "$HOME" claude-code <name> "$CLAUDE_CODE_SESSION_ID"` to remove only that role's registration for this project. If the role has no other registrations left, reset.sh also drops it from the team config. The 4th argument releases any actas exclusivity locks this session held on the role so peers can pick it up immediately (see #62).
...
2. Run: `~/.agents/skills/agmsg/scripts/spawn.sh <type> <name> --project "$HOME" [options]`
...
1. Run: `~/.agents/skills/agmsg/scripts/delivery.sh set off claude-code "$HOME"`
...
1. Run: `~/.agents/skills/agmsg/scripts/reset.sh "$HOME" claude-code`
