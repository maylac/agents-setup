...
Otherwise, run: `~/.agents/agmsg/scripts/whoami.sh "$(pwd)" claude-code`
...
→ Show the user the available teams from the output, then:
  ...
  3. **You MUST use join.sh** — run: `~/.agents/agmsg/scripts/join.sh <team> <agent_name> claude-code "$(pwd)"`
  ...
  5. **REQUIRED — Do NOT skip this step.** Ask the user to pick a delivery mode using exactly this prompt:
     ...
     - Map the chosen number to a mode and run:
       `~/.agents/agmsg/scripts/delivery.sh set <mode> claude-code "$(pwd)"`
...
→ No exact registration exists for this project, but there are same-type agent names registered elsewhere.
  ...
  4. Run: `~/.agents/agmsg/scripts/join.sh <team> <agent_name> claude-code "$(pwd)"`
...
**Ensure monitor is running first.** Before processing any subcommand below, check whether this session already has an `agmsg inbox stream` Monitor task in its TaskList. If not, and the project's delivery mode is `monitor` or `both` (check via `~/.agents/agmsg/scripts/delivery.sh status claude-code "$(pwd)"`), invoke the Monitor tool now:

- command: `~/.agents/agmsg/scripts/watch.sh $CLAUDE_CODE_SESSION_ID "$(pwd)" claude-code`
...
**Sandbox compatibility.** When Claude Code's sandbox is enabled, `watch.sh` (monitor mode) runs inside the sandbox and needs to write pidfiles and SQLite WAL files under `~/.agents/agmsg/`. If monitor mode fails with write/permission errors there, add an allowlist entry to `~/.claude/settings.json` (or project-level `.claude/settings.local.json`):
...
2. Run `~/.agents/agmsg/scripts/identities.sh "$(pwd)" claude-code` to see whether the role is already registered for this (project, type).
3. If the name does not appear in the output, join under the existing team. Read TEAMS from the in-session whoami state (it may be a single team or comma-separated). For a single team, run `~/.agents/agmsg/scripts/join.sh <team> <name> claude-code "$(pwd)"`. For multiple teams, ask the user which team to join the new role into, then run join.sh for that team.
4. **Pre-flight claim** the actas exclusivity lock so this role isn't already owned by another live session: `~/.agents/agmsg/scripts/actas-claim.sh "$(pwd)" claude-code <name> "$CLAUDE_CODE_SESSION_ID"`. Read the `status=` line of the output:
...
5. **Switch receive too — exclusive role mode.**
   ...
   d. Invoke a fresh Monitor regardless of whether step b or c applied:
      - command: `~/.agents/agmsg/scripts/watch.sh $CLAUDE_CODE_SESSION_ID "$(pwd)" claude-code <name>`
...
2. Run `~/.agents/agmsg/scripts/reset.sh "$(pwd)" claude-code <name> "$CLAUDE_CODE_SESSION_ID"` to remove only that role's registration for this project. If the role has no other registrations left, reset.sh also drops it from the team config. The 4th argument releases any actas exclusivity locks this session held on the role so peers can pick it up immediately (see #62).
...
2. Run: `~/.agents/agmsg/scripts/spawn.sh <type> <name> --project "$(pwd)" [options]`
...
2. Determine which team `<name>` belongs to (as with `send`), then run:
   `~/.agents/agmsg/scripts/despawn.sh <team> $AGENT <name> [--force] [--timeout <secs>]`
...
1. Run: `~/.agents/agmsg/scripts/delivery.sh set off claude-code "$(pwd)"`
