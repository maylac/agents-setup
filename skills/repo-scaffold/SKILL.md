---
name: repo-scaffold
description: 新規リポジトリを起こすとき、この環境の標準構成(チェック可能な基準を持つAgent Guide、docs/learnings、必要に応じてBacklog.md)を再現する。「新しいレポジトリを作って」「プロジェクトを立ち上げて」「リポジトリを初期化して」と言われたときに使用する。既存リポジトリへの後付け適用にも使える。
---
