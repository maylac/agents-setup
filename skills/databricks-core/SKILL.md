...
description: "Databricks CLI operations and parent/entry-point skill: authentication, profile selection, and bundles (DABs). Load this first for CLI, auth, profile, and bundle tasks, then load the matching product skill (dbsql, jobs, pipelines, unity-catalog, spark-structured-streaming, aibi-dashboards)."
...
For specific products, use dedicated skills (installed siblings):
...
- **databricks-dbsql** - Databricks SQL advanced features and data modeling
- **databricks-unity-catalog** - governance, access control, and observability
- **databricks-spark-structured-streaming** - production streaming pipelines
- **databricks-aibi-dashboards** - Lakeview AI/BI dashboards

For **data discovery, exploration, and query generation** — finding tables,
answering natural-language questions about the data, or generating SQL — use the
...
These commands are first-class for running known SQL and profiling. For
natural-language data questions, locating data you can't pin down, or generating a
query from a question, see [Manual Data Exploration](manual-data-exploration.md) for the
...
| Exploring tables/schemas | [Manual Data Exploration](manual-data-exploration.md) |
| Deploying jobs/pipelines | Use DABs (Asset Bundles) — see the databricks-jobs / databricks-pipelines skills |
