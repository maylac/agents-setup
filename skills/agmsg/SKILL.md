...
Otherwise, run: `~/.agents/skills/agmsg/scripts/whoami.sh "$PWD" <claude-code|codex per current harness>`
...
2. Run `~/.agents/skills/agmsg/scripts/identities.sh "$PWD" <claude-code|codex per current harness>` to see whether the role is already registered for this (project, type).
...
2. Run `~/.agents/skills/agmsg/scripts/reset.sh "$PWD" <claude-code|codex per current harness> <name>` to remove that role's registration.
...
1. Run: `~/.agents/skills/agmsg/scripts/reset.sh "$PWD" <claude-code|codex per current harness>`
