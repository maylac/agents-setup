...
Run only when the user invokes `/udemy-mockexam-exporter` directly.
...
```
$HOME/workspace/certificate-learning/
...
```bash
cd $HOME/workspace/certificate-learning && \
...
! cd $HOME/workspace/certificate-learning && python3 udemy_login.py
...
```bash
wc -l $HOME/workspace/certificate-learning/<出力ファイル名>.md
head -30 $HOME/workspace/certificate-learning/<出力ファイル名>.md
```
