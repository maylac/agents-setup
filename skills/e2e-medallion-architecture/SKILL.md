...
description: 'Implement Bronze/Silver/Gold medallion lakehouses in Microsoft Fabric: workspace/lakehouse setup (schema-enabled vs legacy), PySpark + Delta ingestion/transform patterns per layer, notebook deployment via REST, pipeline orchestration, and Power BI Direct Lake consumption.'
---
...
Use the microsoft-learn MCP tools (microsoft_docs_search / microsoft_docs_fetch) for current Fabric REST API, authentication/token audiences, notebook-definition (`.ipynb`) structure, `getDefinition`/`updateDefinition` workflow, and Spark optimization details.
...
### MUST DO
- **Choose lakehouse architecture** based on schema-enabled availability (look up Fabric lakehouse infrastructure guidance via microsoft-learn MCP):
...
- Follow the **`.ipynb` validation + Fabric nuances** (look up notebook-definition requirements via microsoft-learn MCP) when creating notebooks via REST API — every code cell must include `"outputs": []` and `"execution_count": null`
...
- Use Materialized Lake Views (MLVs) for Silver/Gold tables when the transformation is expressible in Spark SQL and benefits from declarative refresh semantics (look up Materialized Lake View patterns and incremental refresh via microsoft-learn MCP).
- For MLV refresh scheduling and monitoring (create/delete schedules, trigger on-demand refresh, check job status), look up Fabric MLV operations via microsoft-learn MCP. Note: "materialized view", "spark materialized view", and "MLV" all refer to the same Fabric feature.
...
When setting up a medallion workspace, choose your architecture pattern first (look up Fabric workspace/lakehouse infrastructure guidance via microsoft-learn MCP):
...
4. **Choose transformation approach**:
   ...
   - **Option 4b:** Use Materialized Lake Views (Spark SQL) for declarative transformations with incremental refresh (when query is IR-eligible) — look up Materialized Lake View patterns and incremental refresh via microsoft-learn MCP
...
2. **Bind each notebook to its lakehouse** — set `metadata.dependencies.lakehouse` with the correct lakehouse ID (look up notebook default-lakehouse binding via microsoft-learn MCP):
...
1. **Land data in lakehouse first**: External data must be staged into the lakehouse `Files/` folder before Spark can read it — use one of:
   ...
   - **OneLake API / `curl`** — upload files via REST API using `storage.azure.com` token (look up OneLake data access via microsoft-learn MCP)
...
2. **Create notebooks** — one per layer, with valid `.ipynb` structure (look up notebook-definition requirements via microsoft-learn MCP)
...
6. **Execute notebooks sequentially** — use `POST .../jobs/instances?jobType=RunNotebook`:
   ...
   - Check for recent jobs before submitting (prevent duplicates — look up Fabric Spark job execution via microsoft-learn MCP)
...
2. **Verify Gold tables via SQL** — connect to the SQL endpoint using `sqlcmd` (look up Fabric SQL / TDS data-plane access via microsoft-learn MCP) and confirm the target table exists:
   ```sql
   SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'nyc_taxi_daily_summary'
   ```
3. **Create a semantic model** — create via `POST /v1/workspaces/{workspaceId}/items` with `type: "SemanticModel"` then deploy definition via `updateDefinition` using TMDL format (look up SemanticModel item-definition and TMDL deployment via microsoft-learn MCP):
...
4. **Create a Power BI report** — `POST /v1/workspaces/{workspaceId}/items` with `type: "Report"` then deploy definition via `updateDefinition` using PBIR format (look up Report item-definition and PBIR format via microsoft-learn MCP):
...
- **For semantic model authoring** (TMDL, refresh, permissions), look up Fabric semantic-model authoring via microsoft-learn MCP
- **For DAX query validation**, look up DAX query / semantic-model consumption via microsoft-learn MCP
...
**For detailed Spark configurations and optimization strategies, look up Fabric Spark data-engineering patterns via microsoft-learn MCP.**
...
```bash
# Workspace creation (look up full Fabric REST/CLI patterns via microsoft-learn MCP)
