...
Routing: SOL is the coordinator — decompose, set acceptance criteria, synthesize, judge. Implementation and routine fixes go to the pinned gpt-5.6-terra delegation by default; offload bounded mechanical work (lookup, inventory, summarization, formatting) to Antigravity/Gemini; do not execute delegated implementation in the SOL parent. The daily harness report flags SOL above 50% of token usage as routing drift.
